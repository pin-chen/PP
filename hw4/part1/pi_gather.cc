...
int main(int argc, char **argv)
{
    ...
    // TODO: use MPI_Gather
    uint64_t recv_in_circle[world_size];
    MPI_Gather(&local_in_circle, 1, MPI_UNSIGNED_LONG_LONG, recv_in_circle, 1, MPI_UNSIGNED_LONG_LONG, 0, MPI_COMM_WORLD);
    ...
    if (world_rank == 0)
    {
        ...
        for (int i = 0; i < world_size; i++) {
            in_circle += recv_in_circle[i];
        }
        pi_result = 4.0 * in_circle / tosses;
        // --- DON'T TOUCH ---
